...
public class MusicMgr : BaseMgr<MusicMgr>
{
    ...
    private void UpdateT()
    {
        if (isDelete)
        {
            ...
            {
                var source = ListSoundMusics[i];

                // 检查 AudioSource 是否已被销毁
                if (source == null)
                {
                    ListSoundMusics.RemoveAt(i);
                    continue;
                }
                ...
                {
                    source.clip = null;
                    PoolMgr.Instance.PushInObj(source.gameObject);
                    ListSoundMusics.RemoveAt(i);
                }
            }
        }
    ...
    public void Init()
    {
        ...
        //Debug.Log("MusicMgr 实例ID: " + this.GetHashCode());
        //数据读取校对
    ...
    /// 音效缓存预热
    /// </summary>
    /// <param name="max">最大同时存在音效</param>
    public void SoundPoolStart(int max)
    {
        PoolMgr.Instance.Preload("MusicPlayObj", max, DataAndInitMgr.Instance.resourcesNecessaryAssetsPath);
    }
